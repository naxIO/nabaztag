...
$resultAct = mysqli_query($con2,"SELECT description from buttonAction order by id;");
if (!$resultAct) die('Button picker invalid query. ' . mysqli_error());
...
while($rowAct = mysqli_fetch_row($resultAct))
 {      
   ?>
    <option <?php if($s_button == $rowAct[0]) echo 'selected'; ?> value="<?php echo $rowAct[0];  ?>"><?php echo $rowAct[0];?>
    </option>
<?php }
